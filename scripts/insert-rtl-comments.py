#!/usr/bin/env python3
"""Safely insert verified line comments into RTL with legacy encodings.

Mode 2 must preserve every original source byte except the planned inserted
comments. Ordinary patch tools cannot open many GB18030 RTL sources, so this
small, plan-driven helper provides an encoding-aware write path. It accepts a
UTF-8 JSON plan, requires a pre-edit format capture for each source, and
rejects a plan whose source cannot round-trip to its original bytes.
"""
...
def code_marker_positions(text: str, marker: str) -> list[int]:
    ...
    Historical RTL often retains prior implementations as line comments. A
    source-level marker can therefore appear in active code and in history.
    Line comments cannot form active Verilog tokens, so exclude only those;
    block-comment ambiguity remains a deliberate hard failure.
    """
    positions: list[int] = []
    start = 0
    while True:
...
def shared_capture(entries: list[dict[str, object]], root: Path, source: Path) -> dict[str, object]:
    ...
    return load_capture(next(iter(captures)))


def decoded_source(source: Path, capture: dict[str, object]) -> tuple[bytes, str, str]:
    ...
    return raw, text, encoding


def apply_to_source(source: Path, entries: list[dict[str, object]], root: Path, write: bool) -> dict[str, object]:
    capture = shared_capture(entries, root, source)
    raw, text, encoding = decoded_source(source, capture)
    ...
    for entry in entries:
        ...
        if len(marker_positions) != 1:
            fail(f"active-code marker must occur once in {source} (found {len(marker_positions)}): {marker!r}")
...
def verify_applied_source(source: Path, entries: list[dict[str, object]], root: Path) -> dict[str, object]:
    """Remove exact planned blocks in memory and compare to the pre-edit SHA."""
    capture = shared_capture(entries, root, source)
    raw, text, encoding = decoded_source(source, capture)
